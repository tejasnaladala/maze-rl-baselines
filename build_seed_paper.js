...
const abstractBlock = [
  ...
  abstract(
    "On 9x9 procedural mazes generated by depth-first search, we observe three empirical results worth documenting. First: across seven HP-tuned modern reward-driven configurations (Stable-Baselines3 PPO, DQN, A2C across three learning rates each, 70 runs total), the best baseline (SB3 DQN at default LR) reaches 31.4% test success, statistically tied with a uniform random walk (32.7%); PPO, A2C, and recurrent DQN underperform the random walk by significant margins. A behavior-cloning MLP of identical architecture, observation, and optimizer reaches 97.4%. Second: initializing DQN online and target networks from the BC-distilled weights (which evaluate at 97.2% before fine-tuning) and fine-tuning with the same shaped reward as the from-scratch DQN drops test success to a mean of 18.4% across n=20 seeds (sd 11.5, median 17.0, range 0 to 38), an approximately 79 percentage point drop from the BC starting accuracy. Post-fine-tune mean is statistically tied with from-scratch custom MLP_DQN (19.3%) and 13 percentage points below from-scratch SB3 DQN at default LR (31.4%); the BC initialization does not retain its 97% performance under standard DQN fine-tuning, but the post-fine-tune policy is not reliably below from-scratch DQN as a smaller pilot suggested. Third (added in response to an adversarial audit recommendation): adding a standard count-based intrinsic-motivation bonus to PPO across 20 seeds at 500K env steps each yields mean 9.4% test success (sd 18.2, median 3.0, range 0 to 68), with 10 of 20 seeds collapsing to 0 to 2% and only 2 of 20 reaching above 50%. The exploration-augmented baseline does not close the gap to BC distillation in mean. We frame the BC -> DQN basin instability as a candidate instance of capability loss under reward-driven fine-tuning, with the caveat that this is one fine-tune recipe; broader robustness across LR, exploration schedule, and offline-RL variants (CQL, IQL) is open. The result is preliminary and has known scope limitations, most prominently that DFS-generated mazes are tree graphs on which a wall-following heuristic is provably complete (it terminates at the goal on every instance, though not at shortest-path-optimal step count). The contribution of this note is to document the result, state the scope limits clearly, and identify the experiments that would resolve them. Code, data, manifest, and seeds: github.com/tejasnaladala/maze-rl-baselines."
  )
];
...
const resultsTable = new Table({
  ...
  rows: [
    ...
    dataRow("BC -> DQN (post fine-tune, n=20)",            "18.4%   (sd 11.5, median 17, range 0 to 38)")
  ]
});
...
const findingsRest = [
  ...
  para([t("DQN initialized from BC_MLP weights begins evaluation at 97.2% test success, matching BC_MLP within bootstrap CI. After training under the standard DQN reward signal across n=20 seeds, post-fine-tune test success has mean 18.4% (sd 11.5, median 17.0, range 0 to 38). Per-seed sorted: 0, 0, 4, 8, 8, 12, 14, 14, 16, 16, 18, 20, 22, 24, 26, 26, 28, 36, 38, 38. The drop from BC starting accuracy (97.2%) to post-fine-tune mean (18.4%) is approximately 79 percentage points, consistent across the n=20 sweep. The variance is substantial (6 of 20 seeds collapse to 0 to 8%, 4 of 20 reach 26 to 38%); the post-fine-tune mean is statistically tied with from-scratch custom MLP_DQN (19.3%) and 13 percentage points below from-scratch SB3 DQN at default LR (31.4%). This is the main empirical finding of the note: the BC basin is not preserved by the standard DQN fine-tune procedure, though the resulting policy is not reliably below from-scratch DQN as a smaller pilot had suggested.")]),
