...
- **Single primary maze class.** We use one procedural-maze generator (recursive backtracking with ⌊n/3⌋ hazards). The recursive-backtracking generator produces simply-connected mazes (a single spanning tree, no loops). This topological property is what permits the EgoWallFollowerLeft heuristic to achieve 100 percent. Results may not extend to loopy-maze generators (Aldous-Broder, Wilson with loop injection) where wall-following is not optimal. A pilot on a loopy generator is queued for v1.1.
- **Distillation proves expressibility, not reachability.** The 97.4 percent BFS-distilled MLP demonstrates the network class can express the maze-solving policy as a global minimum of supervised loss. This is not the same as the policy being reachable under reward-driven gradient descent from a random initialization. A behavioral-cloning warm-start experiment (initialize MLP_DQN from distilled weights, then fine-tune via DQN reward) would directly test whether the reward landscape destroys the representation; this is queued for v1.1.
