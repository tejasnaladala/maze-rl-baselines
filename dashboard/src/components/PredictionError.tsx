...
export default function PredictionError({ history }: PredictionErrorProps) {
  ...
  const avg = data.length > 0 ? data.reduce((s,d)=>s+d.error,0)/data.length : 0
  ...
  return (
    <div style={{ width:'100%', height:'calc(100% - 22px)', padding:'2px 4px 2px 0', position:'relative' }}
      ...
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'baseline', gap:'3px' }}>
          <span style={{
            ...
            textShadow:'0 0 10px rgba(240,144,48,0.20)',
          }}>{cur.toFixed(3)}</span>
          <span style={{ fontFamily:'var(--mono)', fontSize:'7px', color:'var(--t-dim)' }}>PE</span>
        </div>
        {showAvg && (
      ...
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top:18, right:4, left:-24, bottom:0 }}>
          <defs>
            <linearGradient id="peG" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#f09030" stopOpacity={0.15} />
              <stop offset="70%" stopColor="#f09030" stopOpacity={0.02} />
              <stop offset="100%" stopColor="#f09030" stopOpacity={0} />
            </linearGradient>
          </defs>
          <XAxis dataKey="tick" tick={{fontSize:7,fill:'#2a3450',fontFamily:'var(--mono)'}}
            tickLine={false} axisLine={{stroke:'rgba(80,120,200,0.05)'}} interval="preserveStartEnd" />
          <YAxis tick={{fontSize:7,fill:'#2a3450',fontFamily:'var(--mono)'}}
            tickLine={false} axisLine={{stroke:'rgba(80,120,200,0.05)'}}
            domain={[0,'auto']} width={30} />
          {showAvg && <ReferenceLine y={avg} stroke="rgba(240,144,48,0.15)" strokeDasharray="3 4" />}
