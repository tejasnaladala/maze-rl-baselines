...
- Single primary maze class (recursive backtracking with hazards). The recursive-backtracking generator produces simply-connected mazes (a single spanning tree, no loops). This topological property is what permits the EgoWallFollowerLeft heuristic to achieve 100 percent. Results may not extend to loopy-maze generators (Aldous-Broder, Wilson) where wall-following is not optimal, nor to partially observable variants beyond DRQN. A pilot on a loopy generator is queued for v1.1.
- Distillation demonstrates the network class can express the maze-solving policy as a global minimum of supervised loss, not that the same minimum is reachable under reward-driven gradient descent from a random initialization. A behavioral-cloning warm-start experiment (initialize RL from distilled weights, then fine-tune via DQN reward) would directly test whether the reward landscape destroys the representation. This is queued for v1.1.
