...
- **Simply-connected maze generator.** Our recursive-backtracking generator produces mazes with a single spanning tree (no loops). This topological property is precisely why EgoWallFollowerLeft achieves 100%. The headline pattern (heuristic strong, distilled MLP strong, neural RL weak) may not extend to loopy-maze generators (Aldous-Broder, Wilson with loop injection) where wall-following is not optimal. A 5-seed pilot on a loopy generator is queued for v1.1 to test this boundary.
