...
- **Distillation proves expressibility, not reachability.** The 97.4% BFS-distilled MLP demonstrates the network class can express the maze-solving policy as a global minimum of supervised loss. This is not equivalent to demonstrating that the same minimum is reachable under reward-driven gradient descent from a random initialization. A behavioral-cloning warm-start experiment (initialize MLP_DQN from distilled weights, then fine-tune via DQN reward signal) would directly disambiguate: if test success holds at ~97% post-fine-tune the representation was reachable; if it collapses back toward ~19% the reward landscape actively destroys the representation. This experiment is implemented and queued for v1.1.
