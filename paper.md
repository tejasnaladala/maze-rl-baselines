...
- **Simply-connected maze generator (loopy pilot run).** Our recursive-backtracking generator produces mazes with a single spanning tree (no loops). To test whether wall-following depends on this topology, we ran a 5-seed pilot on Wilson-algorithm mazes with 4 extra passages added (loops introduced, hazards disabled to isolate topology). **EgoWallFollowerLeft still solves 100% of these loopy mazes**; the heuristic is robust to modest loop injection. MLP_DQN_h64 reaches 66% on the same hazard-free loopy distribution (under-trained at 100 episodes; full-budget number for v1.1). The MLP_DQN jump from 19.3% (hazard-loaded simply-connected) to 66% (hazard-free Wilson + loops) suggests hazard avoidance interacts with topology in ways worth further investigation. A v1.1 follow-up will run the loopy condition with hazards re-enabled and at higher loop densities (60-80% of eligible interior walls).
- **BC warm-start collapse experiment (RAN, n=5).** Behavioral-cloning warm-start was tested to disambiguate "expressibility" from "reachability." Train MLP via BC on BFS-oracle labels (mean BC test 97.2%), init MLP_DQN online + target nets from those weights, fine-tune via standard DQN (200K env steps, eps 0.20→0.05 over 50K). **Per-seed BC→FT: (98→0), (100→18), (98→16), (90→12), (100→22). Mean: 97.2% → 13.6%.** The DQN reward gradient actively destroys the distilled high-performing representation; post-fine-tune ends BELOW the from-scratch MLP_DQN baseline (19.3%). This is a substantially STRONGER claim than the original paper makes: the failure is not merely "RL can't find the high-performing basin from random init" but "RL actively pushes a network out of the high-performing basin even when initialized inside it." Larger-seed (n≥20), longer-budget, and fine-tune-LR sweep variants are queued for v1.1.
